...
export interface RecordRepository {
  ...
  /** Minimal user-requested text search over preserved Record wording. */
  searchText(
    query: string,
    limit: number,
  ): Promise<readonly PersonalRecord[]>;
...
export interface ReflectionEpisodeRepository {
  ...
  /**
   * Episodes about one target, newest first.
   *
   * A read-side capability with no epistemic content: it selects and orders,
   * and decides nothing. It exists because §22 provenance is only useful if it
   * can be read back — without it a Discovery has no path to the reflections
   * it elicited, so a saved reflection could never be shown again after a
   * reload. Precedent: `RecordRepository.listRecent`.
   */
  listByTarget(targetRef: string): Promise<readonly ReflectionEpisode[]>;
}
...
export interface UserReflectionRecordRepository {
  ...
  /**
   * Reflections elicited by one episode, oldest first.
   *
   * Retains superseded entries (INV-12). The episode is the only link from a
   * target to the text the user wrote about it, since a UserReflectionRecord
   * itself carries no targetRef (§24: it wraps a Record, not a claim).
   */
  listByEpisode(
    episodeRef: string,
  ): Promise<readonly UserReflectionRecord[]>;
